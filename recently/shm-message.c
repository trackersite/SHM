...
int shm_message_copy(shm_message_t message_source, shm_message_t *message_target) {
  if (!shm_message_is_empty(*message_target)) {
    printf("./shm-message.c:%s:%d: Unable to copy the message because the target message is not empty.\n", __FILE__, __LINE__);
    return -1;
  }
...
int shm_message_set_name(shm_message_t *message, const char *name) {
  if (strlen(name) > 16) {
    ...
    printf("./shm-client.out:%s:%d: This message name is a bit too long to be accepted. That is sad. ", __FILE__, __LINE__);
    printf("Because its \"%d\" size is greater than \"16\".\n", textSize);
    exit(1);
  }
...
int shm_message_set_text(shm_message_t *message, const char *text) {
  if (strlen(text) > 64) {
    ...
    printf("./shm-client.out:%s:%d: This message text is a bit too long to be accepted. That is sad. ", __FILE__, __LINE__);
    printf("Because its \"%d\" size is greater than \"64\".\n", textSize);
    exit(1);
  }
